...
def indent_characters(
    depth: int = 1,
) -> str:  # return the correct indentation for a given indent depth
  indent_symbols = "  "
...
def create_function_signature(
    name: str, return_type: str, arguments: Optional[Dict[str, str]]
) -> str:
...
def create_function_definition(
  ...
  return (
      create_function_signature(name, return_type, arguments)
      + " {\n"
      + indent_code(code)
      + "\n}"
  )
...
def create_exception_declaration(
    name: str, base_class: str = "std::exception"
) -> str:
...
def create_class_definition(
  ...
  def inheritance_helper(
      inheritance_type: str, inheritance: Optional[List[str]]
  ) -> str:
    if not inheritance:
      return ""
  ...
  def member_helper(member_type: str, members: Optional[List[str]]) -> str:
    if not members:
      return ""
  ...
  inheritance = (
      public_inheritance or protected_inheritance or private_inheritance
  )
  if inheritance:
    ret = ret + " : "
