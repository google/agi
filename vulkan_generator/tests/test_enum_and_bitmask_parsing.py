...
Because the XML is irregular, unfortunately the test structure is also
irregular.
...
def test_enum_with_value_fields() -> None:
  ...
  assert (
      vulkan_enum.fields["VK_COMMAND_BUFFER_LEVEL_PRIMARY"].representation
      == "0"
  )
  assert (
      vulkan_enum.fields["VK_COMMAND_BUFFER_LEVEL_PRIMARY"].parent
  ...
  assert (
      vulkan_enum.fields["VK_COMMAND_BUFFER_LEVEL_SECONDARY"].representation
      == "1"
  )
  assert (
      vulkan_enum.fields["VK_COMMAND_BUFFER_LEVEL_SECONDARY"].parent
      == "VkCommandBufferLevel"
  )
...
def test_enum_with_bitmask_fields() -> None:
  ...
  assert (
      vulkan_enum.fields["VK_COMMAND_POOL_CREATE_TRANSIENT_BIT"].representation
      == "0x00000001"
  )
...
def test_enum_with_64bit_bitmask_fields() -> None:
  ...
  assert (
      vulkan_enum.fields["VK_ACCESS_2_INDIRECT_COMMAND_READ_BIT"].representation
      == "0x00000001ULL"
  )

  assert field_names[1] == "VK_ACCESS_2_SHADER_SAMPLED_READ_BIT"
  assert (
      vulkan_enum.fields["VK_ACCESS_2_SHADER_SAMPLED_READ_BIT"].value == 2**32
  )
  assert (
      vulkan_enum.fields["VK_ACCESS_2_SHADER_SAMPLED_READ_BIT"].representation
      == "0x100000000ULL"
  )
...
def test_enum_with_both_value_and_bitmask_fields() -> None:
  ...
  assert (
      vulkan_enum.fields["VK_CULL_MODE_FRONT_BIT"].representation
      == "0x00000001"
  )

  assert vulkan_enum.fields["VK_CULL_MODE_FRONT_AND_BACK"].value == 3
  assert (
      vulkan_enum.fields["VK_CULL_MODE_FRONT_AND_BACK"].representation
      == "0x00000003"
  )
