...
def convert_render_pass_allowance(
    allowance: Optional[str],
) -> types.RenderpassAllowance:
...
def convert_cmd_buffer_level(
    levels: Optional[List[str]], new_types: types.VulkanTypeInfo
) -> Optional[List[types.VulkanEnumField]]:
...
def convert_vk_result_codes(
    codes: Optional[List[str]], new_types: types.VulkanTypeInfo
) -> Optional[List[types.VulkanEnumField]]:
  ...
  return [
      query.get_enum_field_or_deducted_field(results, code) for code in codes
  ]


def convert_queues(
    queues: Optional[List[str]], new_types: types.VulkanTypeInfo
) -> Optional[List[types.VulkanEnumField]]:
...
def process(
  ...
  for command in internal_commands.commands.values():
    ...
    for param in command.parameters.values():
      ...
      typ = query.get_type_or_deducted_type(
          parser_utils.get_plain_typename(param.parameter_type), vulkan_types
      )
    ...
    levels = convert_cmd_buffer_level(
        command.command_buffer_levels, vulkan_types
    )
