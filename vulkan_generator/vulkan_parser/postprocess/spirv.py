...
def process(
    ...
    vulkan_extensions: Dict[str, types.VulkanExtension],
) -> types.SpirvMetadata:
  ...
  for spirv_extension in internal_metadata.extensions.values():
    ...
    if spirv_extension.version:
      ...
      version_str = spirv_extension.version.replace(
          "VK_API_VERSION", "VK_VERSION"
      )
  ...
  for spirv_capability in internal_metadata.capabilities.values():
    ...
    if spirv_capability.feature:
      feature_struct = query.get_struct_or_deducted_struct(
          spirv_capability.feature.struct, vulkan_types
      )
    ...
    if spirv_capability.property:
      ...
      if isinstance(property_group.type, types.VulkanBitmask):
        ...
        if not value_enum:
          raise ValueError(
              f"Unexpected bitmask: {property_group.name} :"
              f" {property_group.type.typename}"
          )
        ...
        raise ValueError(
            f"Unexpected property group: {property_group.name} :"
            f" {property_group.type.typename}"
        )

      new_property = types.SpirvCapabilityProperty(
          struct=property_struct, group=property_group, value=property_value
      )
