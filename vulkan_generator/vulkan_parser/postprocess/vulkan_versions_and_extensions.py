...
class RequiredInfo:
  ...
  command_info: types.VulkanCommandInfo = field(
      default=types.VulkanCommandInfo()
  )
...
) -> types.VulkanFeatureSet:
  """Post process Vulkan Features

  This will process all the linking and group the required/added features to
  their types
  """
  ...
  for feature in features.values():
    if feature.feature_type == "type":
      if feature.name in required_info.type_info.all_types:
        new_feature_set.types[feature.name] = required_info.type_info.all_types[
            feature.name
        ]
      elif feature.name in required_info.type_info.all_aliases:
        new_feature_set.type_aliases[feature.name] = (
            required_info.type_info.all_aliases[feature.name]
        )
      elif feature.name in required_info.includes:
        new_feature_set.includes[feature.name] = required_info.includes[
            feature.name
        ]
      elif feature.name in required_info.defines:
        new_feature_set.defines[feature.name] = required_info.defines[
            feature.name
        ]
      else:
      ...
      if feature.name in required_info.command_info.commands:
        new_feature_set.commands[feature.name] = (
            required_info.command_info.commands[feature.name]
        )
      elif feature.name in required_info.command_info.command_aliases:
        new_feature_set.command_aliases[feature.name] = (
            required_info.command_info.command_aliases[feature.name]
        )
      else:
        raise ValueError(f"Unknown required type: {feature}")
      ...
      if isinstance(feature, internal_types.VulkanFeatureExtensionDefine):
        ...
        new_feature_set.enum_fields[feature.name] = extended_enum.fields[
            feature.name
        ]
      elif isinstance(feature, internal_types.VulkanFeatureExtensionEnumAlias):
        extended_enum = required_info.type_info.enums[feature.basetype]
        new_feature_set.enum_field_aliases[feature.name] = (
            extended_enum.aliases[feature.name]
        )
      else:
...
def process_requirements(
    ...
    required_info: RequiredInfo,
) -> Union[str, List[types.VulkanExtensionRequirement]]:
  """Post Process extension requirements.

  If any linked extension cannot be found, then this function will return the
  name of it instead
  """
  ...
  for requirement in internal_requirements:
    ...
    new_requirements.append(
        types.VulkanExtensionRequirement(
...
def process_extension(
    ...
    new_extensions: Dict[str, types.VulkanExtension],
    required_info: RequiredInfo,
) -> Union[types.VulkanExtension, str]:
  ...
  promotedto: Optional[
      Union[types.VulkanCoreVersion, types.VulkanExtension]
  ] = None
  ...
  requirements = process_requirements(
      extension.requirements, new_extensions, required_info
  )
...
def copy_circular_dependent_extension(
  ...
  new_extensions[extension.name].required_extensions = (
      extension.required_extensions
  )
...
def process_extension_with_dependencies(
  ...
  new_extension = process_extension(
      internal_extensions[extension_name], new_extensions, required_info
  )

  if isinstance(new_extension, str):
    process_extension_with_dependencies(
        new_extension, internal_extensions, new_extensions, required_info
    )
    process_extension_with_dependencies(
        extension_name, internal_extensions, new_extensions, required_info
    )
  else:
...
def process_extensions(
...
) -> Dict[str, types.VulkanExtension]:
  ...
  for extension in internal_extensions.values():
    ...
    process_extension_with_dependencies(
        extension.name, internal_extensions, new_extensions, required_info
    )
...
def process_core_versions(
    ...
    new_types: types.VulkanTypeInfo,
    new_commands: types.VulkanCommandInfo,
) -> Dict[str, types.VulkanCoreVersion]:
