...
def process_enums(
    internal_enums: Dict[str, internal_types.VulkanEnum],
) -> Dict[str, types.VulkanEnum]:
  """Post process enums
...
def process_union(
    internal_union: internal_types.VulkanUnion, new_types: types.VulkanTypeInfo
) -> Union[types.VulkanUnion, str]:
  ...
  for member in internal_union.members.values():
    ...
    fullname = parser_utils.get_full_static_array_name(
        member.member_name, member.size
    )
...
def process_funcptr(
...
) -> Union[str, types.VulkanFunctionPtr]:
  ...
  plain_return_typename = parser_utils.get_plain_typename(
      internal_funcptr.return_type
  )
...
def process_struct(
  ...
  for member in internal_struct.members.values():
    ...
    if member.expected_value:
      expected_value = new_types.enums[member.member_type].fields[
          member.expected_value
      ]

    size: Optional[
        Union[types.VulkanDefine, types.VulkanStructMember, List[int]]
    ] = None
    static = True
    ...
    if member.c_bitfield_size:
      full_member_name = parser_utils.get_full_c_bitfield_name(
          member.member_name, member.c_bitfield_size
      )
    elif static:
      full_member_name = parser_utils.get_full_static_array_name(
          member.member_name, member.size
      )
...
def process_type_with_dependencies(
  ...
  if typename in unions:
    ...
    if isinstance(new_union, str):
      process_type_with_dependencies(
          new_union, unions, funcptrs, structs, new_types, new_defines
      )
      process_type_with_dependencies(
          typename, unions, funcptrs, structs, new_types, new_defines
      )
    else:
  ...
  if typename in funcptrs:
    ...
    if isinstance(new_funcptr, str):
      process_type_with_dependencies(
          new_funcptr, unions, funcptrs, structs, new_types, new_defines
      )
      process_type_with_dependencies(
          typename, unions, funcptrs, structs, new_types, new_defines
      )
  ...
  if typename in structs:
    ...
    if isinstance(new_struct, str):
      process_type_with_dependencies(
          new_struct, unions, funcptrs, structs, new_types, new_defines
      )
      process_type_with_dependencies(
          typename, unions, funcptrs, structs, new_types, new_defines
      )
...
def process_structs_unions_and_funcptrs(
  ...
  for union in unions.values():
    if union.typename not in new_types.unions:
      ...
      process_type_with_dependencies(
          union.typename, unions, funcptrs, structs, new_types, new_defines
      )
  ...
  for funcptr in funcptrs.values():
    if funcptr.typename not in new_types.funcpointers:
      ...
      process_type_with_dependencies(
          funcptr.typename, unions, funcptrs, structs, new_types, new_defines
      )
  ...
  for struct in structs.values():
    ...
    if struct.typename not in new_types.structs:
      ...
      process_type_with_dependencies(
          struct.typename, unions, funcptrs, structs, new_types, new_defines
      )
...
def process(
  ...
  for bitmask_alias in internal_vulkan_types.bitmask_aliases.values():
    new_types.bitmask_aliases[bitmask_alias.typename] = (
        types.VulkanBitmaskAlias(
