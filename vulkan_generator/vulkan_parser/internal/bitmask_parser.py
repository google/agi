...
Vulkan bitmasks are special types that they are basically typedefed integer
values
...
def parse_bitmask_by_attribute(
    bitmask_elem: ET.Element,
) -> internal_types.VulkanBitmaskAlias:
  ...
  return internal_types.VulkanBitmaskAlias(
      typename=name, aliased_typename=alias
  )


def parse_bitmask_by_tag(
    bitmask_elem: ET.Element,
) -> internal_types.VulkanBitmask:
  ...
  """
  bitmask_name = parser_utils.get_text_from_tag_in_children(
      bitmask_elem, "name"
  )
  ...
  bitfield_basetype = parser_utils.get_text_from_tag_in_children(
      bitmask_elem, "type"
  )
