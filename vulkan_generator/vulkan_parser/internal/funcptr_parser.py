...
def parse_arguments(
    function_ptr_elem: ET.Element,
) -> Dict[str, internal_types.VulkanFunctionArgument]:
  ...
  for elem in function_ptr_elem:
    ...
    if not elem.text:
      raise SyntaxError(
          f"Argument type could not found: {ET.tostring(elem, 'utf-8')!r}"
      )

    if not elem.tail:
      raise SyntaxError(
          f"Argument name could not found: {ET.tostring(elem, 'utf-8')!r}"
      )
    ...
    if argument_name.count("const") > 1:
      raise SyntaxError(
          f"Double const are not supported: {argument_type} {argument_name}"
      )

    # Multiple pointers are not supported
    if argument_name.count("*") > 1:
      raise SyntaxError(
          f"Double pointers are not supported: {argument_type} {argument_name}"
      )
    ...
    if "const" in argument_name:
      if not argument_name.endswith("const"):
        raise SyntaxError(
            f"""This is probably a const pointer which is not supported:
...
def parse(func_ptr_elem: ET.Element) -> internal_types.VulkanFunctionPtr:
  ...
  function_name = parser_utils.get_text_from_tag_in_children(
      func_ptr_elem, "name"
  )
  ...
  if not return_type:
    raise SyntaxError(
        "No return type found for the function pointer:"
        f" {ET.tostring(func_ptr_elem, 'utf-8')!r}"
    )
  ...
  return internal_types.VulkanFunctionPtr(
      typename=function_name, return_type=return_type, arguments=arguments
  )
