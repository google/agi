...
def parse_format_planes(
    format_element: ET.Element,
) -> List[internal_types.ImageFormatPlane]:
  """Parser for the image format planes if format is multi-planar

  Sample image format plane:
  <plane index="0" widthDivisor="1" heightDivisor="1"
  ...
  for plane in format_element:
    ...
    if plane.tag == "plane":
      ...
      planes.append(
          internal_types.ImageFormatPlane(
              index=index,
      ...
      raise SyntaxError(
          f"Uknown image format element{ET.tostring(plane, 'utf-8')!r}"
      )

  return planes


def parse_format_components(
    format_element: ET.Element,
) -> Dict[str, internal_types.ImageFormatComponent]:
  ...
  for component in format_element:
    ...
    if component.tag == "component":
      ...
      raise SyntaxError(
          f"Uknown image format element{ET.tostring(component, 'utf-8')!r}"
      )
...
def parse_format(format_element: ET.Element) -> internal_types.ImageFormat:
  ...
  <format name="VK_FORMAT_G8_B8_R8_3PLANE_420_UNORM" class="8-bit 3-plane 420"
      ...
      <plane index="0" widthDivisor="1" heightDivisor="1"
      compatible="VK_FORMAT_R8_UNORM"/>
      <plane index="1" widthDivisor="2" heightDivisor="2"
      compatible="VK_FORMAT_R8_UNORM"/>
      <plane index="2" widthDivisor="2" heightDivisor="2"
      compatible="VK_FORMAT_R8_UNORM"/>
  </format>
  """
