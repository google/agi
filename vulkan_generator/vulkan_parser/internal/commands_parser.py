...
def parse_arguments(
...
) -> Dict[str, internal_types.VulkanCommandParam]:
  ...
  for param_elem in command_elem:
    ...
    parameter_type = parser_utils.get_text_from_tag_in_children(
        param_elem, "type"
    )
    parameter_name = parser_utils.get_text_from_tag_in_children(
        param_elem, "name"
    )
    ...
    pointers = parser_utils.try_get_tail_from_tag_in_children(
        param_elem, "type"
    )
    # some times it's just empty space or endline character
    if pointers:
    ...
    if array_size_reference:
      ...
      array_size_reference = parser_utils.clean_type_string(
          array_size_reference
      )
...
def parse_command(command_elem: ET.Element) -> internal_types.VulkanCommand:
  ...
  success_codes = parser_utils.try_get_attribute_as_list(
      command_elem, "successcodes"
  )
  error_codes = parser_utils.try_get_attribute_as_list(
      command_elem, "errorcodes"
  )
  queues = parser_utils.try_get_attribute_as_list(command_elem, "queues")
  command_buffer_levels = parser_utils.try_get_attribute_as_list(
      command_elem, "cmdbufferlevel"
  )

  renderpass_allowance = command_elem.get("renderpass")

  name = parser_utils.get_text_from_tag_in_children(command_elem[0], "name")
  return_type = parser_utils.get_text_from_tag_in_children(
      command_elem[0], "type"
  )
...
def parse_command_alias(
    command_elem: ET.Element,
) -> internal_types.VulkanCommandAlias:
  ...
  return internal_types.VulkanCommandAlias(
      command_name=name, aliased_command_name=alias
  )
...
def parse(commands_elem: ET.Element) -> internal_types.AllVulkanCommands:
  ...
  for command_elem in commands_elem:
    ...
    if "alias" in command_elem.attrib:
      ...
      vulkan_commands.command_aliases[command_alias.command_name] = (
          command_alias
      )
      continue
