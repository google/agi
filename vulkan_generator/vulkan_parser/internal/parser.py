...
def process_enums(
    vulkan_types: internal_types.AllVulkanTypes, enum_element: ET.Element
) -> None:
  ...
  if not vulkan_enums:
    raise SyntaxError(
        f"Enum could not be parsed {ET.tostring(enum_element, 'utf-8')}"
    )
...
def process_core_versions(
    versions: Dict[str, internal_types.VulkanCoreVersion],
  ...
  if not features:
    raise SyntaxError(
        "Vulkan version could not be parsed"
        f" {ET.tostring(feature_element, 'utf-8')!r}"
    )
...
def get_enum_field_for_extension(
    extension: internal_types.VulkanFeatureExtensionEnum, bit64: bool
) -> Optional[parser_utils.EnumFieldRepresentation]:
  ...
  if extension.offset:
    return parser_utils.get_enum_field_from_offset(
        extnumber_str=extension.extnumber,
        offset_str=extension.offset,
        sign_str=extension.sign,
    )
...
def append_extended_feature(
  ...
  if isinstance(feature, internal_types.VulkanFeatureExtensionEnum):
    field = get_enum_field_for_extension(
        feature, vulkan_types.enums[feature.basetype].bit64
    )

    if not field:
      raise SyntaxError(
          f"Enum field for {feature.basetype}:{feature.name} could not be"
          " generated"
      )

    vulkan_types.enums[feature.basetype].fields[feature.name] = (
        internal_types.VulkanEnumField(
            name=feature.name,
            value=field.value,
    ...
    vulkan_types.enums[feature.basetype].aliases[feature.name] = (
        internal_types.VulkanEnumFieldAlias(
...
def process_spirv_extensions(
    spirv_metadata: internal_types.SpirvMetadata,
    spirv_extensions_elem: ET.Element,
) -> None:
  ...
  for extension_element in spirv_extensions_elem:
    ...
    if not spirv_extension:
      raise SyntaxError(
          "Spirv Extension could not found:"
          f" {ET.tostring(extension_element, 'utf-8')}"
      )
...
def process_spirv_capabilities(
    spirv_metadata: internal_types.SpirvMetadata, spirv_cap_elem: ET.Element
) -> None:
  ...
  for child in spirv_cap_elem:
    ...
    if not spirv_capability:
      raise SyntaxError(
          "Spirv Capability could not found:"
          f" {ET.tostring(spirv_cap_elem, 'utf-8')!r}"
      )
