...
def parse(feature_element: ET.Element) -> internal_types.VulkanCoreVersion:
  ...
  for require_element in feature_element:
    if require_element.tag != "require":
      raise SyntaxError(
          "Unknown Tag in Vulkan features"
          f" {ET.tostring(require_element, 'utf-8')!r}"
      )

    requirement = parser_utils.parse_requirement(
        require_element=require_element
    )
    features.update(requirement.features)

  return internal_types.VulkanCoreVersion(
      name=version_name, number=version_number, features=features
  )
