...
def parse(
    extensions_element: ET.Element,
) -> Dict[str, internal_types.VulkanExtension]:
  ...
  for extension_element in extensions_element:
    if extension_element.attrib["supported"] not in ["vulkan", "disabled"]:
      raise SyntaxError(
          f"Unexpected Support: {ET.tostring(extension_element, 'utf-8')!r}"
      )
    ...
    required_extensions = parser_utils.try_get_attribute_as_list(
        extension_element, "requires"
    )
