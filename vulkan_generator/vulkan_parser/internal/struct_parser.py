...
def parse_struct_members(
    struct_element: ET.Element,
) -> Dict[str, internal_types.VulkanStructMember]:
  """Parses a Vulkan Struct member

  This is a bit of an irregular code because the XML itself has quite
  irregularities that
  ...
  for member_element in struct_element:
    ...
    member_type = parser_utils.get_text_from_tag_in_children(
        member_element, "type"
    )
    member_name = parser_utils.get_text_from_tag_in_children(
        member_element, "name"
    )
    ...
    bitfield_str = parser_utils.try_get_tail_from_tag_in_children(
        member_element, "name"
    )
...
def parse(struct_elem: ET.Element) -> internal_types.VulkanType:
  ...
  <type category="struct" name="VkDevicePrivateDataCreateInfo"
      ...
      <name>privateDataSlotRequestCount</name></member>
  </type>
  ...
  if alias_name:
    return internal_types.VulkanStructAlias(
        typename=struct_name, aliased_typename=alias_name
    )

  base_structs = parser_utils.try_get_attribute_as_list(
      struct_elem, "structextends"
  )
