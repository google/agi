...
It parses all defines except some of the defines that are defined under enums as
"API Constants"
"""
...
def parse_define_by_attribute(
  ...
  if not value:
    raise SyntaxError(
        "Vulkan define does not have definition:"
        f" {ET.tostring(define_elem, 'utf-8')!r}"
    )
...
def parse_define_by_tag(define_elem: ET.Element) -> internal_types.VulkanDefine:
  ...
  if not name:
    raise SyntaxError(
        f"Define name could not be parsed: {ET.tostring(define_elem, 'utf-8')}"
    )

  if not value:
    raise SyntaxError(
        f"Define value could not be parsed: {ET.tostring(define_elem, 'utf-8')}"
    )
...
def parse(define_elem: ET.Element) -> internal_types.VulkanDefine:
  ...
  <type category="define" requires="VK_MAKE_API_VERSION">// Vulkan 1.1 version
  number
