...
def get_text_from_tag_in_children(
    elem: ET.Element, tag: str, recursive: bool = False
) -> str:
  """Gets the text of the element with the given tag in element

  Args: recursive(bool): If True: The function will search the entire subtree
  recursively including the root node If False: The function will search only
...
def try_get_text_from_tag_in_children(
    elem: ET.Element, tag: str, recursive: bool = False
) -> Optional[str]:
...
def get_tail_from_tag_in_children(
    elem: ET.Element, tag: str, recursive: bool = False
) -> str:
...
def try_get_tail_from_tag_in_children(
    elem: ET.Element, tag: str, recursive: bool = False
) -> Optional[str]:
  """Tries to Gets the text of the element with the given tag
...
def try_get_attribute_as_list(
    elem: ET.Element, attrib: str
) -> Optional[List[str]]:
...
def clean_type_string(string: str) -> str:
  ...
  return (
      ...
      .replace(",", "")
      .replace(");", "")
      .replace("conststruct", "const struct")
  )
...
def get_plain_typename(string: str) -> str:
  ...
  """
  return clean_type_string(
      string.replace("const", "").replace("*", "").replace("struct", "")
  )
...
def get_enum_field_from_bitpos(
    bitpos_str: str, bit64: bool
) -> EnumFieldRepresentation:
...
def get_enum_field_from_offset(
    extnumber_str: Optional[str], offset_str: str, sign_str: Optional[str]
) -> EnumFieldRepresentation:
...
def parse_requirement(
    require_element: ET.Element,
) -> internal_types.VulkanExtensionRequirement:
  ...
  for required_feature_element in require_element:
    ...
    if required_feature_element.tag != "enum":
      ...
      features[feature_name] = internal_types.VulkanFeature(
          name=feature_name, feature_type=feature_type
      )
      continue
...
def parse_static_array(member_element: ET.Element) -> Optional[List[str]]:
  ...
  if not size:
    raise SyntaxError(
        "No static array reference found:"
        f" {ET.tostring(member_element, 'utf-8')}"
    )
  ...
  if size == "[":
    ...
    if not size:
      raise SyntaxError(
          f"Unexpected array size, {ET.tostring(member_element, 'utf-8')}"
      )
...
def parse_dynamic_array(member_element: ET.Element) -> Optional[List[str]]:
  ...
  if not size:
    raise SyntaxError(
        "No dynamic array reference found:"
        f" {ET.tostring(member_element, 'utf-8')}"
    )
