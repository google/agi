...
`gapit` executable in your AGI install location. The default install location
varies by platform:
<span class="info">`C:\Program Files (x86)\agi\gapit.exe` on Windows, `/Applications/AGI.app/Contents/MacOS/gapit` on Mac OS X, or `/opt/agi/gapit` on linux.</span>
...
| [coarse_profile](#coarse_profile)|Profile a replay to get the time of executing the commands.
...
| [framegraph](#framegraph)|Get the frame graph of a capture
...
| [perfetto](#perfetto)|Run metrics and interact with system profiler trace.
| [profile](#profile)|Profile a replay to get GPU activity and counter data.
...
| [server_performance](#server_performance)|Test the performance of GAPIS replay generation.
| [split](#split)|Splits traces by carving out subranges into new traces
...
| [validate_gpu_profiling](#validate_gpu_profiling)|Validates the GPU profiling capability of a device
...
GAPIT-flags:
  ...
  -experimental-enable-angle-tracing 
	Enable the experimental feature ANGLE tracing.
  -experimental-enable-perf-tab 
	Enable the experimental feature Counter performance tab.
  -experimental-enable-vulkan-tracing 
	Enable the experimental feature Vulkan tracing.
  -log-file string
...
**Usage:**  GAPIT [GAPIT-flags] coarse_profile [coarse_profile-flags]
```
GAPIT-flags:
  -analytics string
	If non-empty enable analytics using the specified user-id
...
coarse_profile-flags:
  ...
  -gapis-token string
	The auth token to use when connecting to an existing server.
  -loopcount int
	The number of times to loop the trace. (experimental) (default 1)
  -out string
...
commands-flags:
  ...
  -observations-typedobservations 
	if true then display the bytes read and written by each command as resolved types
  -onlyexecuteddraws 
	Only show executed draw calls from within command buffers
  -raw 
...
dump_pipeline-flags:
  -at value
	command/subcommand index to get the pipeline after, e.g. '[123, 0, 0, 4]'. Empty for last
  -captureid 
	if true then interpret the capture file argument as a capture ID that is already loaded in gapis
  -compute 
...
dump_resources-flags:
  -at int
	command index to dump the resources after, e.g. '1234' (default -1)
  -captureid 
	if true then interpret the capture file argument as a capture ID that is already loaded in gapis
  -gapir-args string
	The arguments to be passed to gapir
...
export_replay-flags:
  ...
  -loopcount int
	The number of times to loop the trace. (experimental) (default 1)
  -mode value
	generate special purposed trace [one of: "plain", "on-screen", "diagnostics", "frames", "timestamps"] (default plain)
  -originaldevice 
...
*Get the frame graph of a capture*

**Usage:**  GAPIT [GAPIT-flags] framegraph [framegraph-flags]
```
GAPIT-flags:
  -analytics string
	If non-empty enable analytics using the specified user-id
...
framegraph-flags:
  -dot string
	Store the framegraph in Graphviz dot format in this file
  -gapis-args string
	The arguments to be passed to gapis
  -gapis-disablelog 
	Disable the log output
  ...
  -gapis-token string
	The auth token to use when connecting to an existing server.
  -json string
	Store the framegraph in JSON format in this file
```
[Back](#gapit-help)
#  make_doc
...
memory-flags:
  -at value
	command/subcommand index to get the memory after, e.g. '[123, 0, 0, 4]'. Empty for last
  -captureid 
	if true then interpret the capture file argument as a capture ID that is already loaded in gapis
  -gapis-args string
	The arguments to be passed to gapis
...
*Run metrics and interact with system profiler trace.*

**Usage:**  GAPIT [GAPIT-flags] perfetto [perfetto-flags]
```
GAPIT-flags:
  -analytics string
	If non-empty enable analytics using the specified user-id
...
perfetto-flags:
  -categories string
	Comma separated list of metric categories from the input file. Only valid if 'metrics' mode is selected.
  -format value
	Output file format: {text|json}. [one of: "default", "text", "json"] (default default)
  -gapis-args string
	The arguments to be passed to gapis
  -gapis-disablelog 
	Disable the log output
  ...
  -gapis-token string
	The auth token to use when connecting to an existing server.
  -in string
	Input file. Refer to documentation for file format.
  -mode value
	Run mode: {metrics|interactive}. Default: metrics. [one of: "metrics", "interactive", "list"] (default metrics)
  -out string
	Output file.
```
[Back](#gapit-help)
...
*Profile a replay to get GPU activity and counter data.*
...
profile-flags:
  -disableaf 
	Disable Anisotropic Filtering for all samplers
  -disabledcmds value
	command/subcommand index (e.g. '[123, 0, 0, 4]') for disabling a draw call (repeatable)
  -gapir-args string
	The arguments to be passed to gapir
  -gapir-device string
	Device to use. Either 'host' or the friendly name of the device
  ...
  -gapis-token string
	The auth token to use when connecting to an existing server.
  -json 
	Return replay profiling data as JSON instead of text
  -out string
	Output file (optional, if none then output goes to stdout)
```
[Back](#gapit-help)
#  replace_resource
...
replace_resource-flags:
  -at int
	command index to replace the resource(s) at, e.g. '1234' (default -1)
  -captureid 
	if true then interpret the capture file argument as a capture ID that is already loaded in gapis
  -gapir-args string
	The arguments to be passed to gapir
  ...
  -gapis-token string
	The auth token to use when connecting to an existing server.
  -handle string
	required. handle or ID of the resource to replace
...
screenshot-flags:
  -at value
	command/subcommand index (e.g. '[123, 0, 0, 4]') for the screenshot (repeatable)
  -attachment string
  ...
  -executeddraws int
	create a screenshot at every (total-draw-calls / this-flag-value) drawcall, e.g. for 100 draw calls, '-executeddraws 4' takes a screenshot at every 25th call
  -frame value
...
*Test the performance of GAPIS replay generation.*

**Usage:**  GAPIT [GAPIT-flags] server_performance [server_performance-flags]
```
GAPIT-flags:
  -analytics string
	If non-empty enable analytics using the specified user-id
...
server_performance-flags:
  ...
  -gapis-token string
	The auth token to use when connecting to an existing server.
  -loopcount int
	The number of times to loop the trace. (experimental)
  -originaldevice 
	export replay for the original device
```
[Back](#gapit-help)
...
*Splits traces by carving out subranges into new traces*

**Usage:**  GAPIT [GAPIT-flags] split [split-flags]
```
GAPIT-flags:
  -analytics string
	If non-empty enable analytics using the specified user-id
...
split-flags:
  -captureid 
	if true then interpret the capture file argument as a capture ID that is already loaded in gapis
  -from uint
	The inclusive start index of the command range. Default: 0 (first command)
  -gapir-args string
	The arguments to be passed to gapir
  -gapir-device string
	Device to use. Either 'host' or the friendly name of the device
  ...
  -out string
	Output file.
  -to uint
	The exclusive end index of the command range. Default: 0 (last command)
```
[Back](#gapit-help)
#  state
...
state-flags:
  -at value
	command/subcommand index to get the state after, e.g. '[123, 0, 0, 4]'. 0 for first command. Empty for last
  -captureid 
	if true then interpret the capture file argument as a capture ID that is already loaded in gapis
  -depth int
	How many nodes deep should the state tree be displayed. -1 for all (default -1)
  -filter value
	Which path (e.g. '[root, Devices]') through the tree should we filter to, default All
  -gapir-args string
	The arguments to be passed to gapir
  -gapir-device string
	Device to use. Either 'host' or the friendly name of the device
...
trace-flags:
  -additionalargs string
	additional arguments to pass to the application
  -api string
	only capture the given API valid options are vulkan, angle and perfetto
  -capture-frames int
  ...
  -device string
	Device to use. Either 'host' or the friendly name of the device
  -disable-coherentmemorytracker 
	disables the coherent memory tracker so it won't interfere with gdb during trace
  ...
  -perfetto string
	File containing the Perfetto configuration proto.
  -pipename string
...
*Validates the GPU profiling capability of a device*

**Usage:**  GAPIT [GAPIT-flags] validate_gpu_profiling [validate_gpu_profiling-flags]
```
GAPIT-flags:
  -analytics string
	If non-empty enable analytics using the specified user-id
...
video-flags:
  ...
  -type value
	type of output to produce [one of: "auto", "sxs", "regular", "frames", "sxs-frames"] (default auto)
```
[Back](#gapit-help)
