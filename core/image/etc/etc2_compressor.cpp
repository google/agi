...
Etc::Image::Format convert_etc_format(etc_format format) {
    switch(format) {
        case ETC2_RGB_U8_NORM: return Etc::Image::Format::RGB8;
        case ETC2_RGBA_U8_NORM: return Etc::Image::Format::RGBA8;
        case ETC2_RGBA_U8U8U8U1_NORM: return Etc::Image::Format::RGB8A1;
        case ETC2_SRGB_U8_NORM: return Etc::Image::Format::SRGB8;
        case ETC2_SRGBA_U8_NORM: return Etc::Image::Format::SRGBA8;
        case ETC2_SRGBA_U8U8U8U1_NORM: return Etc::Image::Format::SRGB8A1;
        case ETC2_R_U11_NORM: return Etc::Image::Format::R11;
        case ETC2_RG_U11_NORM: return Etc::Image::Format::RG11;
        case ETC2_R_S11_NORM: return Etc::Image::Format::SIGNED_R11;
        case ETC2_RG_S11_NORM: return Etc::Image::Format::SIGNED_RG11;
        case ETC1_RGB_U8_NORM: return Etc::Image::Format::ETC1;
        default: return Etc::Image::Format::UNKNOWN;
    }
}

void read_image(const uint8_t* input_image, uint32_t width, uint32_t height, Etc::ColorFloatRGBA* output) {
    ...
    for (uint32_t h = 0; h < height; ++h) {
        ...
        for (uint32_t w = 0; w < width; ++w) {
            *dst = Etc::ColorFloatRGBA::ConvertFromRGBA8(src[0], src[1], src[2], src[3]);
            dst++;
            src += BYTE_PER_PIXEL;
        }
    }
...
extern "C" etc_error compress_etc(
    ...
    Etc::Image image(reinterpret_cast<float*>(source_image), width, height, ERROR_METRIC);
    image.m_bVerboseOutput = false;

    auto image_format = convert_etc_format(format);
    if(image_format == Etc::Image::Format::UNKNOWN) {
        return static_cast<etc_error>(Etc::Image::EncodingStatus::ERROR_UNKNOWN_FORMAT);
    }

    auto status = image.Encode(image_format, ERROR_METRIC, EFFORT, MIN_JOBS, MAX_JOBS);
...
extern "C" char*  get_etc_error_string(etc_error error_code) {
    ...
    if(status > Etc::Image::EncodingStatus::WARNING_THRESHOLD) {
        written = sprintf(current, "\"Warning with the Encoding Status Bits: %d\"", status);
        current += written;
    }
