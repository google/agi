...
func (b *stateWatcher2) OnBeginCmd(ctx context.Context, cmdID api.CmdID, cmd api.Cmd) {
}
func (b *stateWatcher2) OnEndCmd(ctx context.Context, cmdID api.CmdID, cmd api.Cmd) {
}
func (b *stateWatcher2) OnBeginSubCmd(ctx context.Context, subIdx api.SubCmdIdx, recordIdx api.RecordIdx) {
}
func (b *stateWatcher2) OnRecordSubCmd(ctx context.Context, recordIdx api.RecordIdx) {
}
func (b *stateWatcher2) OnEndSubCmd(ctx context.Context) {
}
func (b *stateWatcher2) OnReadFrag(ctx context.Context, owner api.RefObject, frag api.Fragment, valueRef api.RefObject, track bool) {
}
func (b *stateWatcher2) OnWriteFrag(ctx context.Context, owner api.RefObject, frag api.Fragment, oldValueRef api.RefObject, newValueRef api.RefObject, track bool) {
}

func (b *stateWatcher2) OnWriteSlice(ctx context.Context, slice memory.Slice) {
...
func (b *stateWatcher2) OnReadSlice(ctx context.Context, slice memory.Slice) {
}
func (b *stateWatcher2) OnWriteObs(ctx context.Context, observations []api.CmdObservation) {
}
func (b *stateWatcher2) OnReadObs(ctx context.Context, observations []api.CmdObservation) {
}
func (b *stateWatcher2) OpenForwardDependency(ctx context.Context, dependencyID interface{}) {
}
func (b *stateWatcher2) CloseForwardDependency(ctx context.Context, dependencyID interface{}) {
}
func (b *stateWatcher2) DropForwardDependency(ctx context.Context, dependencyID interface{}) {
}

type backupMemory2 struct {
...
type loopingVulkanControlFlowGenerator struct {
	...
	memoryForStagingBuffer map[VkDevice]*backupMemory2
...
func NewLoopingVulkanControlFlowGenerator(ctx context.Context, chain *transform.TransformChain, out transform.Writer, graphicsCapture *capture.GraphicsCapture, loopStart api.CmdID, loopEnd api.CmdID, loopCount int32) controlFlowGenerator.ControlFlowGenerator {
	...
	return &loopingVulkanControlFlowGenerator{
		...
		memoryForStagingBuffer: make(map[VkDevice]*backupMemory2),
...
func (f *loopingVulkanControlFlowGenerator) TransformAll(ctx context.Context) error { // ALAN: TODO: f -> cf
	...
	for !f.chain.IsEndOfCommands() {
		...
		cmdId := id.GetID() // TODO: ALAN: Handle End ID, also rename this function! We don't need more Id.GetID.id()
		...
		if cmds, err := f.chain.ProcessNextTransformedCommands(subctx); err == nil {
			...
			if lastObservedCommand == api.CmdNoID || lastObservedCommand < f.loopStartIdx {
				...
				if api.CmdID.Real(cmdId) >= f.loopEndIdx && cmdId != api.CmdNoID {
					...
					// // If we are looping zero times we can just drop the commands inside the loop. // ALAN: TODO: Handle
					// if f.loopCount == 0 {
...
func (f *loopingVulkanControlFlowGenerator) allocateMemoryForStagingbuffer(ctx context.Context, stateBuilder *stateBuilder) {
	...
	for buffer := range f.bufferChanged {
		...
		if _, ok := f.memoryForStagingBuffer[dev]; !ok {
			f.memoryForStagingBuffer[dev] = &backupMemory2{VkDeviceMemory(0), VkDeviceSize(0), VkDeviceSize(0)}
		}
